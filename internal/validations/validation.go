		...
		for _, e := range validationError {
			...
			case "iso_datetime":
				errors[fieldPath] = fmt.Sprintf(
					"%s phải đúng định dạng ISO 8601 (ví dụ: 2024-12-31T23:59:59Z hoặc 2024-12-31T23:59:59+07:00)",
					fieldPath,
				)
			case "url":
				errors[fieldPath] = fmt.Sprintf("%s phải là URL hợp lệ (ví dụ: https://example.com)", fieldPath)
			}
		}
